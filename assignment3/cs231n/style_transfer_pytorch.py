...
def content_loss(content_weight, content_current, content_original):
    ...
    loss = content_weight * torch.norm(content_current - content_original)**2
...
def gram_matrix(features, normalize=True):
    ...
    for n in range(N):
        for i in range(C):
            for j in range(C):
                gram[n][i][j] = torch.dot(f[n][i], f[n][j])
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, style_ind in enumerate(style_layers):
        style_layer = gram_matrix(feats[style_ind])
        style_target = style_targets[i]
        style_weight = style_weights[i]
        
        loss += content_loss(style_weight, style_layer, style_target)
...
def tv_loss(img, tv_weight):
    ...
    # this loss fn sucks eggs
    horiz = torch.norm(img[:,:,:,1:]-img[:,:,:,:-1])**2
    vert = torch.norm(img[:,:,1:,:]-img[:,:,:-1,:])**2
    
    return tv_weight * (horiz + vert)
