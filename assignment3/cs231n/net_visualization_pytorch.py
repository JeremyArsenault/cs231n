...
def compute_saliency_maps(X, y, model):
    ...
    ones_tensor = torch.ones_like(scores)
...
def make_fooling_image(X, target_y, model):
    ...
    while not fooled:
        ...
        scores[0][target_y].view(1).backward(one)
        
        with torch.no_grad(): 
            dX = X_fooling.grad / torch.norm(X_fooling.grad)
            X_fooling +=  learning_rate * dX
        
        fooled = (target_y == torch.argmax(scores))
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    (scores[0][target_y].view(1)+reg*l2_reg).backward(one)

    with torch.no_grad(): 
        dX = img.grad / torch.norm(img.grad)
