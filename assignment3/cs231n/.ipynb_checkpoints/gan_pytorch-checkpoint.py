...
def logistic(logit):
    """
    Computes the inverse of logit function - maps (-inf, inf) back to (0,1)
    """
    return torch.pow(logit, 2) / (1 + torch.pow(logit, 2))
...
def generator_loss(logits_fake):
    ...
    ones = torch.ones_like(logits_fake).type(torch.float32)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    ones = torch.ones_like(scores_fake).type(torch.float32)

    loss = 0.5 * torch.pow(scores_real - ones, 2).mean() + 0.5 * torch.pow(scores_fake, 2).mean()
...
def ls_generator_loss(scores_fake):
    ...
    loss = 0.5 * torch.pow(scores_fake - ones, 2).mean()
